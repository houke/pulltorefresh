...
import { FooterStatus } from 'hangry-react-native-spring-scrollview/LoadingFooter';
import { NormalHeader } from 'hangry-react-native-spring-scrollview/NormalHeader';
import { NormalFooter } from 'hangry-react-native-spring-scrollview/NormalFooter';
import type { HeaderStatus } from 'hangry-react-native-spring-scrollview/RefreshHeader';
import { idx } from 'hangry-react-native-spring-scrollview/idx';
import type {
  Offset,
  SpringScrollViewPropType,
} from 'hangry-react-native-spring-scrollview/Types';
import { styles } from 'hangry-react-native-spring-scrollview/styles';
...
export class SpringScrollView extends React.PureComponent<SpringScrollViewPropType> {
  ...
  _programaticallyTrigger() {
    if (this.props.refreshing === true) {
      setTimeout(() => {
        this.scrollTo({ x: 0, y: -this.props.refreshHeader.height });
        this._offsetYValue = -this.props.refreshHeader.height;
        this._toRefreshStatus('refreshing');
      });
    } else {
      this.endRefresh();
      this.scrollTo({ x: 0, y: 0 });
      this._offsetYValue = 0;
